...
import { susunBarisTur, KOLOM_TUR, KONTEKS_TUR } from '@/lib/ekspor-permintaan';
...
/**
 * Mengunduh daftar permintaan tur sebagai Excel atau PDF.
 *
...
export async function GET(req: NextRequest) {
  ...
  const semua = await getTourRequests();
  const baris = susunBarisTur(saringPermintaan(semua, filter));

  // Tanpa kolom rupiah, jadi tidak ada yang bergantung pada peran.
  const berkas =
    format === 'pdf'
      ? await berkasPdf(baris, KOLOM_TUR, filter, false, KONTEKS_TUR)
      : await berkasXlsx(baris, KOLOM_TUR, false, KONTEKS_TUR);
  ...
  return new NextResponse(new Uint8Array(berkas), {
    headers: {
      ...
      'Content-Disposition': `attachment; filename="${namaBerkas(filter, format).replace('pesanan-lians', 'permintaan-tur-lians')}"`,
